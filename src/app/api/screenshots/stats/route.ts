...
export async function GET(req: NextRequest) {
  try {
    ...
    const [total, todayCount, flaggedCount, totalStorage, thumbnailStorage] = await Promise.all([
      orgData.screenshot.count({ where: { organizationId: orgId } }),
      orgData.screenshot.count({ where: { organizationId: orgId, capturedAt: { gte: todayStart } } }),
      orgData.screenshot.count({ where: { organizationId: orgId, flagged: true } }),
      orgData.screenshot.aggregate({ where: { organizationId: orgId }, _sum: { fileSize: true } }),
      ...
      orgData.screenshot.aggregate({ where: { organizationId: orgId }, _sum: { thumbnailSize: true } }),
    ]);

    // Recent screenshots per employee (last 7 days)
    const sevenDaysAgo = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000);
    const recentByEmployee = await orgData.screenshot.groupBy({
      by: ['employeeId'],
