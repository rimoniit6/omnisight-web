...
try {
  ...
  // Force light theme via localStorage (next-themes key), then reload
  await page.evaluate(() => {
    localStorage.setItem('theme', 'light')
    localStorage.setItem('worklens-theme', 'light')
  ...
  await page.screenshot({ path: join(OUT, '28-dashboard-light-mode.png'), fullPage: true })
  console.log('saved light mode dashboard')

  // Also capture login in light
  await page.evaluate(() => { localStorage.clear() })
  await page.goto('http://localhost:3000/', { waitUntil: 'networkidle', timeout: 30000 })
  await page.waitForTimeout(2000)
  await page.screenshot({ path: join(OUT, '29-login-light.png'), fullPage: true })
  console.log('saved login light')
} catch (e) {
